...
ID = "H38C"
HYPOTHESIS = (
    "MIRROR CONTROL for H38 (flat on the FEEDBACK side, gradient only on feedforward edges). "
...
def _asym_surrogate(z: torch.Tensor) -> torch.Tensor:
    """MIRROR CONTROL: ``g(z) = 0 for z <= -M ; tanh((z+M)/T) for z > -M``.

    The reflection of H38: here it is the comfortably-FEEDBACK edges that receive zero
    gradient, and the whole budget goes to widening already-correct margins. If H38's gain
    were a side effect of "any one-sided shape" or of the smaller position scale such a shape
    induces, this arm would gain too. Q05 predicts the opposite (alignment -1.30..-3.72).
    """
    above = torch.tanh((z + M) / T)
    return torch.where(z <= -M, torch.zeros_like(z), above)
