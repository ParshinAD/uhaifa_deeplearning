...
ID = "H38D"
HYPOTHESIS = (
    "SCALE CONTROL for H38: plain sigmoid at beta x4 for the whole run (the A-SCALE/H03 axis). "
    "Original H38 text follows. One-sided ASYMMETRIC surrogate g(z)=1 for z>=M, 1+tanh((z-M)/T) for z<M with (M,T)=(0.75,1.5): "
...
def _asym_surrogate(z: torch.Tensor) -> torch.Tensor:
    """SCALE CONTROL: the PLAIN sigmoid with beta multiplied by ``BETA_FACTOR``.

    Q01 established that F depends only on the product ``beta*std``, so this sweeps exactly the
    axis a position-scale change would — the already-killed A-SCALE / H03 axis. H03 only ramped
    beta over the last 25% of epochs; this applies the factor for the WHOLE run, which is the
    stronger version of that control. If H38's gain is reproducible here, it is not the
    asymmetry. On the proxies this control reached at most +0.076 pp (mouse) / +0.051 pp
    (hard synthetic) vs H38's +0.40/+0.63.
    """
    return torch.sigmoid(BETA_FACTOR * z)
