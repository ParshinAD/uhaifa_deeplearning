...
| **A-VIOL** | **"Spend gradient only on violated constraints" — the principle behind H38, generalized beyond a surrogate swap** | **NOW** | H38 (2026-08-17) is a GRAPH-DEPENDENT win: connectome **+0.3668 pp** (9x the gate, the largest pure-Rocket gain in the project), microns **-0.6689 pp**. Controls rule out the two obvious confounds: the mirror shape loses -2.889 pp and a whole-run `beta x4` sigmoid loses -0.580 pp. Gradient mass on feedback edges 0% / 20.6% / 100% maps to -2.889 / 0 / +0.367 pp. Four sub-ideas (density-scaled margin, violation-weighted sampling, margin-as-schedule, and **compose with the H35 sift**) in `backlog.md` § A-VIOL. Gate every one through `mfas.analysis.surrogate_gate` first. | `backlog.md` |
| A-ALT | Alternate discrete<->gradient refinement (TODO 3) | low-med (below A-SUB) | **Gated 2026-08-18** (`dr_tmp/kick_gate.py` had never actually been run). Q01's vise is confirmed on a *sifted* order for the first time: a sigmoid kick at the operating scale drags the 83.9035% order down **-0.1444 pp**, while above the crossover the gradient is dead (-0.0074). H38's one-sided surrogate drifts **2.5x less**. After the attribution control (the same re-sift with NO kick = +0.0032 pp), the kick is worth **+0.0179 pp** — real but single-seed, and structurally it is perturb-and-repair, the family H31 already killed. | `backlog.md` |
...
| **A-SUB** | Partial / stochastic sift ("SGD on part of the neurons", TODO 2) | **PROTOTYPE PASSED (2026-08-18) -> promote to H39** | Disambiguated and gated. The *continuous* reading stays unbuilt (block-coordinate ascent shares the fixed set of full-gradient ascent = the inert dynamics axis; H13 cost -0.84 pp). The *discrete* recast — move each sift mover FULLY with probability p instead of ALL movers a fraction alpha — **beats the shipped H35 rebuild by +0.0116 pp (n=5, sigma 0.0009, all seeds above it) at ~20% less wall**, and converges to a true 1-opt fixed point (0 movers) where alpha=0.7 leaves 74. Randomisation breaks the simultaneity that causes H35's limit cycle; damping only shrinks it. | `backlog.md` |
| A-SURR | Alternate surrogate (flat top/bottom, slower-decaying tanh, TODO 7) | **done — KILLED as H37/H37B (2026-08-17)** | Ran properly rather than by analogy, and the analogy was right for the wrong reason. `tanh` **is** the sigmoid (2.2e-16) and "slower-decaying tanh" is the sigmoid at a different β (the killed H03/A-SCALE axis); a hard flat top/bottom is the killed H11 form. The one untested axis — the **tail exponent** — passed the theory gate (Q04) and then failed everywhere: prototype −0.22/−2.83 pp (mouse/hard synthetic), connectome **−0.45 pp (H37) / −0.93 pp (H37B)** over 3 seeds. Produced `diagnosis.md` § Q04: at **fixed core width** a heavier tail aligns better and scores strictly worse; across widths the statistic merely re-labels core width (= the killed β axis). ⚠ The stronger "alignment anti-correlates with score" reading was **retracted** — it holds only inside the symmetric family and is falsified by H38 (see A-VIOL). | `backlog.md` |
