...
| A-SCALE | Scale/temperature annealing — push positions above the surrogate crossover (std≳459) so the surrogate becomes discriminative | low | Suggested by **Q01**: the best order out-surrogates Rocket only above std≈459, but Rocket free-scales to std≈141 (where its own order is locally optimal). Two prior kills predict failure: growing scale alone won't relocate to a *different* order, and above the crossover σ saturates ⇒ vanishing gradient (H19 soft-rank **stall**, H03 β-schedule **kill**). The discrete sift (H30/H35) already achieves what this gropes toward. | `backlog.md` |
